...
describe("Windows gait-analysis parity", () => {
  it("uses the same three joint definitions and same-side IC cycles", () => {
    const result = analyzeGait(walkingFrames(), { cameraSide: "left" });
    ...
    expect(result.events.every((event) => event.side === "left")).toBe(true);
    ...
    expect(new Set(result.angles.map((point) => point.side))).toEqual(new Set(["left"]));
  ...
  it("provides hidden 4/6/8 Hz validation without changing the clinical result", () => {
    const result = analyzeGait(walkingFrames(5), { validationMode: true, cameraSide: "left" });
    expect(new Set(result.filterValidation.map((item) => item.cutoffHz))).toEqual(new Set([4, 6, 8]));
    expect(result.filterValidation.every((item) => Number.isFinite(item.peakTimingShiftMs))).toBe(true);
    expect(result.filterValidation.every((item) => item.rawFrameToFrameVariation >= 0
      && item.filteredFrameToFrameVariation >= 0)).toBe(true);
  ...
  it("is deterministic and never overwrites raw Pose input", () => {
    ...
    expect(analyzeGait(frames, { cameraSide: "left" })).toEqual(analyzeGait(frames, { cameraSide: "left" }));
  ...
  it("requires an explicit camera-side and does not expose far-side clinical angles", () => {
    const unspecified = analyzeGait(walkingFrames(5));
    expect(unspecified.primarySide).toBeNull();
    expect(unspecified.angles).toEqual([]);
    expect(unspecified.acquisitionQuality.status).toBe("retake");
    const left = analyzeGait(walkingFrames(5), { cameraSide: "left" });
    expect(left.angles.length).toBeGreaterThan(0);
    expect(left.angles.every((point) => point.side === "left")).toBe(true);
  });

  it("adds an ankle caution when an orthosis is used without changing the angle definition", () => {
    const result = analyzeGait(walkingFrames(5), { cameraSide: "left", orthosis: "used" });
    const ankle = result.jointQuality.find((quality) => quality.joint === "ankle_dorsiflexion");
    expect(ankle?.warnings.some((warning) => warning.includes("装具・靴"))).toBe(true);
    expect(result.versions.angleDefinitionVersion).toBe("sagittal-pixel-v2");
  });

  it("recommends retaking a portrait video after automatic metadata review", () => {
    const result = analyzeGait(walkingFrames(5), { cameraSide: "left", videoMetadata: {
